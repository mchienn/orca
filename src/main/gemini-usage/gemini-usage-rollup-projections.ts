...
} from './gemini-usage-scope-filters'
...
export function buildSummary(
  ...
  return {
    ...
    topModel,
    topProject,
    hasAnyGeminiData: filteredSessions.length > 0 || filteredDaily.length > 0
  }
}
...
export function buildDaily(
  ...
  const byDay = new Map<string, GeminiUsageDailyPoint>()
...
export function buildBreakdown(
  ...
  const rows = new Map<string, GeminiUsageBreakdownRow>()
  const filteredDaily = getFilteredDaily(state, scope, range)
  const filteredSessions = getFilteredSessions(state, scope, range)
